...
public sealed class MusicDataProxyController(
    ...
    IFetchArtistInfoUseCase ucFetchArtistInfo) : ControllerBase
{
    private readonly ILogger _logger = loggerProvider.CreateLogger(nameof(MusicDataProxyController));
    
    [HttpGet("artist")]
    public async Task<ActionResult<OutArtist?>> GetArtistInfoAsync(string name, CancellationToken ct = default)
    {
