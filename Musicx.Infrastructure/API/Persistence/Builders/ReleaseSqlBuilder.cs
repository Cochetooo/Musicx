...
internal sealed class ReleaseSqlBuilder(ILoggerProvider loggerProvider) : SqlBuilder<InRelease>
{
    ...
    internal override string BuildSelect(IQuerySpecification<InRelease>? querySpecification = null,
        bool distinct = false)
    {
        if (querySpecification is not ReleaseQuerySpecification releaseQuerySpecification)
        {
            return distinct
    ...
    internal override string BuildGroupBy(IQuerySpecification<InRelease>? spec = null)
